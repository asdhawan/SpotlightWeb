...
namespace CommonUtils {
    public static class Logger {
        ...
        public static void LogInfo(string source, string message, bool bAsync = false, string loginId = null) { LogInfoMessage(CommonEnums.LogMessageType.INFO, source, message, bAsync, loginId); }

        public static void LogDebug(string source, string message, bool bAsync = false, string loginId = null) { LogInfoMessage(CommonEnums.LogMessageType.DEBUG, source, message, bAsync, loginId); }

        public static void LogWarn(string source, string message, bool bAsync = false, string loginId = null) { LogInfoMessage(CommonEnums.LogMessageType.WARN, source, message, bAsync, loginId); }

        public static void LogError(string source, string message, Exception ex, bool bAsync = false, string loginId = null) { LogErrorMessage(CommonEnums.LogMessageType.ERROR, source, message, ex, bAsync, loginId); }

        public static void LogFatal(string source, string message, Exception ex, bool bAsync = false, string loginId = null) { LogErrorMessage(CommonEnums.LogMessageType.FATAL, source, message, ex, bAsync, loginId); }
        ...
        private static void LogInfoMessage(DateTime timestamp, CommonEnums.LogMessageType messageType, string source, string message, string loginId = null) {
            ...
            Log4NetLogger.Info($"[{source}]: {message}");
        ...
        private static void LogErrorMessage(DateTime timestamp, CommonEnums.LogMessageType messageType, string source, string message, Exception ex, string loginId = null) {
            ...
            Log4NetLogger.Error($"[{source}]: {message}", ex);
            ...
            if (ex.InnerException != null) {
                Log4NetLogger.Error($"[{source}]: {message} [Inner Exception]", ex.InnerException);
                Utils.WriteToEventLog(
                    source,
